...
write.csv(legislative_age, "legislative_age.csv", row.names = FALSE)
...
# We calculate the age of the representatives for each year of service
legislative_age$age <- legislative_age$start_year - legislative_age$year_of_birth

# Group by year and gen_cohort, then summarize to calculate median age and count
cohort_summary <- legislative_age %>%
  group_by(start_year, gen_cohort) %>%
  summarise(
...
# Calculate the total number of representatives per year to get proportions
total_counts <- legislative_age %>%
  group_by(start_year) %>%
  summarise(total = n(), .groups = 'drop')

# Merge to calculate proportions
cohort_summary <- cohort_summary %>%
  left_join(total_counts, by = "start_year") %>%
  mutate(share = count / total)

# Now, we plot the share of each cohort against the median age
ggplot(cohort_summary, aes(x = median_age, y = share, color = gen_cohort)) +
  geom_line() +
  scale_y_continuous(labels = scales::percent_format()) +
  labs(
    title = "Generational Control of the House",
    x = "Median Age",
    y = "Share of House of Representatives (%)"
  ) +
  theme_minimal()
